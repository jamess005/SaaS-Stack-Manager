...
def test_full_pipeline_switch(tmp_path):
    ...
    result = run_agent(inbox, outputs_dir=tmp_path / "out", register_path=tmp_path / "hold.json")
...
def test_stay_no_hold_register_entry(tmp_path):
    ...
    run_agent(inbox, outputs_dir=tmp_path / "out", register_path=register_path)
...
def test_full_pipeline_hold(tmp_path):
    ...
    register_path = tmp_path / "hold.json"
    result = run_agent(inbox, outputs_dir=tmp_path / "out", register_path=register_path)
...
def test_invalid_inbox_filename_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="Cannot parse category"):
        run_agent(bad_file, outputs_dir=tmp_path / "out", register_path=tmp_path / "hold.json")


def test_inbox_file_not_found_raises(tmp_path):
    with pytest.raises(FileNotFoundError):
        run_agent(tmp_path / "nonexistent.md", outputs_dir=tmp_path / "out", register_path=tmp_path / "hold.json")
...
def test_load_model_not_called_in_dry_run(tmp_path):
    ...
    with patch("agent.agent.load_model") as mock_load:
        ...
        run_agent(inbox, outputs_dir=tmp_path / "out", register_path=tmp_path / "hold.json")
